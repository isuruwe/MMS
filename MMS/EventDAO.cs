...
public class EventDAO
{
    ...
    private static string connectionString = "data source=135.22.210.105;initial catalog = MMS; user id = mmsuser; password=password;MultipleActiveResultSets=True;App=EntityFramework" ;
    ...
    {   
        ...
        SqlCommand cmd = new SqlCommand("SELECT event_id,clinic_id, description, title, event_start, event_end, all_day FROM Clinic_Schedule where event_start>=@start AND event_end<=@end", con);
        ...
        using (con)
        {
            ...
            while (reader.Read())
            {
                events.Add(new CalendarEvent() {
                    ...
                    clinic= Convert.ToString(reader["clinic_id"]),
    ...
    public static void updateEvent(int id, String title, String description, String clinic)
    {
        SqlConnection con = new SqlConnection(connectionString);
        SqlCommand cmd = new SqlCommand("UPDATE Clinic_Schedule SET title=@title, description=@description,clinic_id=@clinic_id WHERE event_id=@event_id", con);
        cmd.Parameters.Add("@title", SqlDbType.VarChar).Value = title;
        cmd.Parameters.Add("@clinic_id", SqlDbType.VarChar).Value = clinic;
    ...
    {
        SqlConnection con = new SqlConnection(connectionString);
        SqlCommand cmd = new SqlCommand("UPDATE Clinic_Schedule SET event_start=@event_start, event_end=@event_end, all_day=@all_day WHERE event_id=@event_id", con);
    ...
    {
        SqlConnection con = new SqlConnection(connectionString);
        SqlCommand cmd = new SqlCommand("DELETE FROM Clinic_Schedule WHERE (event_id = @event_id)", con);
        cmd.Parameters.Add("@event_id", SqlDbType.Int).Value = id;
    ...
    public static string addEvent(CalendarEvent cevent)
    {
        //add event to the database and return the primary key of the added event row

        //insert
        SqlConnection con = new SqlConnection(connectionString);
        SqlCommand cmd = new SqlCommand("INSERT INTO Clinic_Schedule(title, description,clinic_id, event_start, event_end, all_day) VALUES(@title, @description,@clinic_id, @event_start, @event_end, @all_day)", con);
        ...
        cmd.Parameters.Add("@clinic_id", SqlDbType.VarChar).Value = cevent.clinic;
        ...
        using (con)
        {
            ...
            cmd = new SqlCommand("SELECT max(event_id) FROM Clinic_Schedule where title=@title AND description=@description AND event_start=@event_start AND event_end=@event_end AND all_day=@all_day", con);
            cmd.Parameters.Add("@title", SqlDbType.VarChar).Value = cevent.title;
            cmd.Parameters.Add("@description", SqlDbType.VarChar).Value = cevent.description;
            cmd.Parameters.Add("@event_start", SqlDbType.DateTime).Value = cevent.start;
            cmd.Parameters.Add("@event_end", SqlDbType.DateTime).Value = cevent.end;
